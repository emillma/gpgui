...
def set_all(template: Template, key: list, value, checker=lambda x: isinstance(x, str)):
    if isinstance(key, str):
        key = key.split(".")
    if len(key) == 1:
        if checker(template[key[0]]):
            template[key[0]] = value


def configure_plotly():
    mytemplate = pio.templates["plotly_dark"]
    mytemplate.layout.update(
        plot_bgcolor=colors.gray_shades[9],
        paper_bgcolor=colors.transparent,
        colorway=colors.defaults,
    )
    set_all(mytemplate, "font.family", "'Inter', sans-serif")

    mytemplate.layout.font.update(
        color=colors.gray_shades[4], family="'Inter', sans-serif"
    )

    mytemplate.layout.xaxis.update(gridcolor=colors.gray_shades[8])
    mytemplate.layout.yaxis.update(gridcolor=colors.gray_shades[8])
